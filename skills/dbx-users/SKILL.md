...
A 401 means the token is missing, expired without a refresh token, or revoked. Stored OAuth credentials refresh automatically when possible; otherwise run `dbx auth status`, re-run `dbx auth login`, or refresh `DBX_CLI_TOKEN`.
